...
public class SlideListView extends ListView {
    ...
    public SlideListView(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.context = context;
        dp2px60 = DensityUtil.dp2px(context, 60);
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
			...
			/*当前模式不允许滑动，则直接返回，交给ListView自身去处理*/
                ...
                if (isSlided) {
                    View delete = itemView.findViewById(R.id.textView_delete);
                    if (ev.getX() > (getMeasuredWidth() - dp2px60) && null != delete)
                        delete.performClick();
                    scrollBack();
                    return false;
                }
			...
			/*此处根据设置的滑动模式，自动获取左侧或右侧菜单的长度*/
                if(this.mode == MOD_BOTH){
                    this.leftLength = dp2px60;
                    this.rightLength = dp2px60;
        ...
        // 否则直接交给ListView来处理dispatchTouchEvent事件
        return super.dispatchTouchEvent(ev);
    }
